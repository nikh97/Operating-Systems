...
public class RR{
	...
	public static void main(String[] args) throws FileNotFoundException{
	    ...
	    RR(processes, detailed);
	}

	public static void RR(ArrayList<Node> processes, boolean detailed){
		...
		while(terminated < processes.size()){
			...
			if(running != null){	
				...
				else if(running.time_running == 0){
			...
			if(running == null && !ready.isEmpty()){
				...
				if(running.time_running == 0){

					running.time_running = random.randomOS(running.b);
					running.io_time = running.time_running * running.m;

				}

				running.burst_time = quantum;

				if(running.burst_time > running.time_running) running.burst_time = running.time_running;

				if (running.burst_time > running.time_remaining) running.burst_time = running.time_remaining;
	...
	public static void status(int cycle, ArrayList<Node> processes){
		...
		for(int i = 0; i < processes.size(); i++){
			...
			if (p.status.equals("running"))
				str.append(String.format("%11s", p.status) + String.format("%5d", p.burst_time));
